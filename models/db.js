/**
 * models/db.js  —  Medishop Pharmacy Billing
...
const DB_PATH  = path.join(DATA_DIR, 'medishop_licenses.db');
let db;
...
async function init() {
  ...
  // FIX: activated_at stores first-ever activation time for trial countdown
  ...
  save();
  console.log('[DB] Medishop DB ready →', DB_PATH);
}
...
function log(keyHash, deviceId, ip, action, result, detail = '') {
  run(
    `INSERT INTO validation_log (key_hash,device_id,ip,action,result,detail,ts) VALUES (?,?,?,?,?,?,?)`,
